...
async def handle_link(message: Message):
    ...
    if platform == "youtube":
        ...
        try:
            ...
            await wait_msg.edit_text(texts.PROBE_ERROR)
            return
        ...
        if thumbnail:
            await message.answer_photo(
                photo=thumbnail,
                caption=f"{title}\n\nВыбери качество:",
                reply_markup=kb.quality_kb(qualities),
            )
        else:
            await message.answer("Выбери качество:", reply_markup=kb.quality_kb(qualities))
        return
...
async def handle_quality_choice(callback: CallbackQuery):
    ...
    status = await callback.message.answer(texts.DOWNLOADING)

    if choice == "audio":
        await do_download(status, user_id, url, "youtube", format_selector=None, audio_only=True)
    else:
        await do_download(status, user_id, url, "youtube", format_selector=choice, audio_only=False)
...
async def do_download(status_message: Message, user_id: int, url: str, platform: str,
    ...
    try:
        ...
        await status_message.answer(texts.DOWNLOAD_ERROR)
        return
